@if ($errors->any())
<div class="alert flex flex-row items-center bg-red-200 p-5 rounded-lg my-5 mb-4" x-data="{ show: false }"
    @keydown.escape="show = false" x-init="() => {
...
@endif
@if (session('error'))
<div class="alert flex flex-row items-center bg-red-200 p-5 rounded-lg my-5 w-full"
...
@endif
@if (session('success'))
<div class="alert flex flex-row items-center bg-green-200 p-5 rounded-lg border-green-300 py-5 mb-4"
    x-data="{ show: false }" x-init="() => {
      setTimeout(() => show = true, 500);
      setTimeout(() => show = false, 5000);
    }" x-show="show" x-description="Notification panel, show/hide based on alert state." @keydown.escape="show = false"
